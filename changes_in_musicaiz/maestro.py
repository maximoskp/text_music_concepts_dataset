...
class Maestro(MusicGenerationDataset):
    ...
    *Hawthorne, C., Stasyuk, A., Roberts, A., Simon, I., Huang, C. Z. A., Dieleman, S., ... & Eck, D. (2018).
    ...
    def tokenize(
        ...
        >>> )
        """
        dataset_path = str(Path(dataset_path, dataset_version))
        metadata = self.get_metadata(dataset_path, dataset_version=dataset_version)
    ...
    @staticmethod
    def get_metadata(dataset_path: Union[str, Path], dataset_version='maestro-v3.0.0') -> Dict[str, str]:
        """Prepares the metadata json from the Maestro csv."""
        table = pd.read_csv(str(Path(dataset_path, dataset_version+".csv")))
        composers_json = {}
